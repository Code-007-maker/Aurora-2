...
import {
    ...
    LayersIcon, MapPin, Zap, Eye, Loader2, ArrowRight, Users
} from 'lucide-react';
...
function PersonnelTab() {
    ...
    const [role, setRole] = useState('City Admin');
    ...
    const handleInvite = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (res.ok) {
                ...
                setStatus({ type: 'error', msg: data.detail || 'Failed to send invitation' });
            }
        } catch (err: any) {
            setStatus({ type: 'error', msg: 'Network error or backend offline' });
        } finally {
            setIsInviting(false);
        }
    };
    ...
    return (
        <div className="space-y-6">
            <div className="p-4 rounded-xl bg-blue-500/5 border border-blue-500/20 flex gap-3">
                ...
                <div>
                    <p className="text-xs font-bold text-white">Administrative Delegation</p>
                    <p className="text-[11px] text-slate-400 mt-1">As a System Admin, you can invite City Admins. They will receive an automated email with secure credentials to access the Strategic Command Center.</p>
                </div>
            </div>

            <form onSubmit={handleInvite} className="space-y-4 bg-white/[0.02] border border-white/5 rounded-2xl p-6">
                <h4 className="text-xs font-bold text-white uppercase tracking-widest mb-2">Invite New Administrator</h4>
                
                <div className="space-y-2">
                    <label className="text-[10px] text-slate-500 uppercase tracking-widest block ml-1">Email Address</label>
                    <input
                        type="email"
                        required
                        value={email}
                        onChange={e => setEmail(e.target.value)}
                        className="w-full bg-slate-900 border border-white/10 rounded-xl px-4 py-3 text-sm text-white focus:outline-none focus:border-blue-500/50 transition-all"
                        placeholder="admin@city.gov.in"
                    />
                </div>

                <div className="space-y-2">
                    <label className="text-[10px] text-slate-500 uppercase tracking-widest block ml-1">Assigned Role</label>
                    <select
                        value={role}
                        onChange={e => setRole(e.target.value)}
                        className="w-full bg-slate-900 border border-white/10 rounded-xl px-4 py-3 text-sm text-white focus:outline-none focus:border-blue-500/50 transition-all cursor-pointer"
                    >
                        <option value="City Admin">City Admin (Strategic Access)</option>
                ...
                <AnimatePresence>
                    {status.msg && (
                        <motion.div
                            initial={{ opacity: 0, height: 0 }}
                            animate={{ opacity: 1, height: 'auto' }}
                            className={`p-3 rounded-xl border text-xs font-bold flex items-center gap-2 ${status.type === 'success' ? 'bg-emerald-500/10 border-emerald-500/20 text-emerald-400' : 'bg-red-500/10 border-red-500/20 text-red-400'}`}
                        >
                ...
                <button
                    type="submit"
                    disabled={isInviting}
                    className="w-full py-3.5 bg-blue-600 hover:bg-blue-500 disabled:opacity-50 text-white text-xs font-bold rounded-xl shadow-lg shadow-blue-500/20 transition-all flex items-center justify-center gap-2 group"
                >
                    {isInviting ? (
                        <>
                            <Loader2 className="w-4 h-4 animate-spin" />
                            Sending Secure Invitation...
                        </>
                    ) : (
                        <>
            ...
            <div className="rounded-2xl border border-white/5 bg-white/[0.01] p-6">
                <h4 className="text-[10px] font-bold text-slate-500 uppercase tracking-[0.2em] mb-4">Security Protocol</h4>
                <div className="space-y-3">
                    <div className="flex gap-3">
                        ...
                        <p className="text-[11px] text-slate-500">Invitations generate a 12-character high-entropy random password.</p>
                    </div>
                    <div className="flex gap-3">
                        ...
                        <p className="text-[11px] text-slate-500">Credentials are sent via TLS-encrypted SMTP to the recipient.</p>
                    </div>
                    <div className="flex gap-3">
                        ...
                        <p className="text-[11px] text-slate-500">The recipient must rotate their password on initial authentication.</p>
                    </div>
                </div>
            </div>
        </div>
