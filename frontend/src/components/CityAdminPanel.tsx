...
import {
    ...
    LayoutGrid, Wallet, Wrench, AreaChart, Radio, Info, Loader2
} from 'lucide-react';
...
// 6. PERSONNEL & FIELD COMMAND
function PersonnelTab({ wards }: { wards: Ward[] }) {
    const [email, setEmail] = useState('');
    const [selectedWard, setSelectedWard] = useState(wards[0]?.name || '');
    ...
    const handleInvite = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const res = await fetch(`${apiUrl}/auth/invite`, {
                ...
                body: JSON.stringify({ email, role: 'Ward Officer', ward_id: selectedWard })
            });

            const data = await res.json();
            if (res.ok) {
                setStatus({ type: 'success', msg: `Ward Officer invited for ${selectedWard}!` });
                setEmail('');
            } else {
    ...
    return (
        <div className="space-y-6">
            <div className="p-4 rounded-xl bg-indigo-500/5 border border-indigo-500/20 flex gap-3">
                ...
                <div>
                    <p className="text-xs font-bold text-white">Field Command Delegation</p>
                    <p className="text-[11px] text-slate-400 mt-1">Invite Ward Officers to manage tactical response for specific geographical zones. They will have access to localized sensors and pump controls.</p>
                </div>
            </div>

            <form onSubmit={handleInvite} className="space-y-4 bg-white/[0.02] border border-white/5 rounded-2xl p-6">
                <h4 className="text-xs font-bold text-white uppercase tracking-widest mb-2">Invite Ward Officer</h4>
                
                <div className="space-y-2">
                    <label className="text-[10px] text-slate-500 uppercase tracking-widest block ml-1">Officer Email</label>
                    <input
                        type="email"
                        required
                        value={email}
                        onChange={e => setEmail(e.target.value)}
                        className="w-full bg-slate-900 border border-white/10 rounded-xl px-4 py-3 text-sm text-white focus:outline-none focus:border-indigo-500/50 transition-all"
                        placeholder="officer@mcd.gov.in"
                    />
                </div>

                <div className="space-y-2">
                    <label className="text-[10px] text-slate-500 uppercase tracking-widest block ml-1">Assigned Ward</label>
                    <select
                        value={selectedWard}
                        onChange={e => setSelectedWard(e.target.value)}
                        className="w-full bg-slate-900 border border-white/10 rounded-xl px-4 py-3 text-sm text-white focus:outline-none focus:border-indigo-500/50 transition-all cursor-pointer"
                    >
                        {wards.map(w => (
                ...
                <button
                    type="submit"
                    disabled={isInviting}
                    className="w-full py-3.5 bg-indigo-600 hover:bg-indigo-500 disabled:opacity-50 text-white text-xs font-bold rounded-xl shadow-lg shadow-indigo-500/20 transition-all flex items-center justify-center gap-2 group"
                >
                    {isInviting ? (
            ...
            <div className="p-4 rounded-xl border border-white/5 bg-white/[0.01]">
                <h4 className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-3">Recent Personnel Activity</h4>
                <div className="text-center py-8">
                    <p className="text-[11px] text-slate-600 italic">No new personnel additions in this session.</p>
                </div>
            </div>
        </div>
    );
                    ...
                    <div className="flex-1 overflow-y-auto p-5 custom-scrollbar">
                        <AnimatePresence mode="wait">
                            ...
                            >
                                ...
                                {activeTab === 'personnel' && (
                                    <PersonnelTab wards={dynamicWards} />
                                )}
                            </motion.div>
                        </AnimatePresence>
                    </div>
